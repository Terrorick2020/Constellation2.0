...
export const isValidUsername = ( username: string ): [boolean, number | null] => {
  ...
  if ( username.length < MIN_USER_NAME_LEN || username.length > MAX_USER_NAME_LEN ) {
    return [ false, 1 ]
  }
...
export const isValidPassword = ( password: string ): [boolean, number | null] => {
  ...
  if ( password.length < MIN_PASS_INP_LEN || password.length > MAX_PASS_INP_LEN ) {
    return [false, 3]
  }
